...
from mogaze_dataset import MoGazeDataset, MoGazeEval
...
parser.add_argument('--exp-name', type=str, default='mogaze_baseline.txt', help='experiment name')
...
# FIXED: Use the correct config attribute names that match H36M
...
def train_step(mogaze_motion_input, mogaze_motion_target, model, optimizer, nb_iter, total_iter, max_lr, min_lr):
    ...
    # For MoGaze: 22 joints * 3 rotations = 66 (same structure as H36M)
    joints = 22  # MoGaze has 22 joints like H36M
    ...
    writer.add_scalar('Loss/mogaze', loss.detach().cpu().numpy(), nb_iter)
...
dataset = MoGazeDataset(config, 'train', config.data_aug)
...
eval_dataset = MoGazeEval(eval_config, 'test')
...
print("Starting MoGaze training...")

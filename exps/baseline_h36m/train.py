...
def train_step(h36m_motion_input, h36m_motion_target, model, optimizer, nb_iter, total_iter, max_lr, min_lr, dct_m, idct_m, writer):
...
def main():
    ...
    if config.model_pth is not None:
        ...
        print_and_log_info(logger, f"Loading model path from {config.model_pth}")
    ...
    while (nb_iter + 1) < config.cos_lr_total_iters:
        for (h36m_motion_input, h36m_motion_target) in dataloader:
            loss, optimizer, current_lr = train_step(h36m_motion_input, h36m_motion_target,
                                                     model, optimizer, nb_iter,
            ...
            if (nb_iter + 1) % config.print_every == 0:
                avg_loss /= config.print_every
                avg_lr /= config.print_every
            ...
            if (nb_iter + 1) % config.save_every == 0:
                torch.save(model.state_dict(), os.path.join(config.snapshot_dir, f'model-iter-{nb_iter + 1}.pth'))
                model.eval()
                acc_tmp = test(eval_config, model, eval_dataloader)
                print(acc_tmp)
                acc_log.write(f'{nb_iter + 1}\n')
                acc_log.write(' '.join(str(x) for x in acc_tmp) + '\n')
                model.train()
